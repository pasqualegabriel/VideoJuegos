...
export (int) var ray_velocity = 15
...
func _handle_cannon_actions(should_fire:bool = true):
	...
	if is_firing:
		...
		_apply_ray_velocity(mouse_position)
...
func _apply_ray_velocity(mouse_position):
	velocity += (global_position - mouse_position).normalized() * ray_velocity
